...
from run_experiments import (
    generateHistoricalCarbonBudgets,
    extractRequestTraceByYear,
    getConstantsFromBPMN,
    optimizationHourly,
    get_microservice_configs,
...
def generate_pareto_budgets():
    ...
    # Generate HIGH budget for LL config only
    configs = ['LL']
...
def run_selective_optimization_pareto(config, ci_data, clickData_hourly, region, workload,
...
def run_high_performance_baseline_pareto(config, clickData_hourly, region, workload,
...
def run_simple_carbon_aware_baseline_pareto(config, ci_data, clickData_hourly, region, workload,
                                           carbon_budget, result_file, budget_level):
...
def run_sequential_carbon_aware_baseline_pareto(config, ci_data, clickData_hourly, region, workload,
                                               carbon_budget, result_file, budget_level):
...
def run_pareto_experiments():
    ...
    configs = ['LL']  # Only LL configuration
    budget_levels = [100, 90, 80, 70, 60, 50, 40, 30, 20, 10]
