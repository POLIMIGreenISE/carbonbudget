...
for CB_version in range(5):
    ...
    lastConfiguration = "NP" # Inizializzo a NP la variabile che tiene salvata la configurazione usata nell'ora precedente
    currentConfiguration = "NP" # Inizializzo a NP la variabile che tiene salvata la configurazione che scelgo nell'ora corrente

    totRevenue = 0

    row = ["q","user-throughput","ed","QoE","Revenue per user", "Revenue", "Revenue in percentage","eb","U","ce"] # Preparo il file csv
    f = open('results/sequentialCarbonAware/'+ x +'sequentialCarbonAwareBaseline.csv', 'w')
    writer = csv.writer(f)
    writer.writerow(row)
    f.close()
    ...
    for t in range(8760): # Itero per tutte le ore dell'anno 
        ...
        if lastConfiguration == "HP":
            if ed_hp <= eb: # Se l'energia richiesta in HP è minore del budget 
                currentConfiguration = "HP" # Rimango in HP
            else:
                currentConfiguration = "NP" # Altrimenti scalo e passo in NP

        elif lastConfiguration == "NP":
            if ed_hp <= eb: # Se l'energia richiesta in HP è minore del budget 
                currentConfiguration = "HP" # Passo a HP
            elif ed_np <= eb:  # Se l'energia richiesta in NP è minore del budget 
                currentConfiguration = "NP" # Rimango in NP
            else:
                currentConfiguration = "LP"

        elif lastConfiguration == "LP":
            if ed_np <= eb: # Se l'energia richiesta in NP è minore del budget 
                currentConfiguration = "NP" # Scalo in HP
            else:
                currentConfiguration = "LP" # Altrimenti rimango in LP

        lastConfiguration = currentConfiguration

        if currentConfiguration == "HP": # Se scelgo la modalità HP
        ...
        elif currentConfiguration == "NP": # Se scelgo la modalità NP
        ...
        elif currentConfiguration == "LP": # Se scelgo la modalità LP
        ...
        f = open('results/sequentialCarbonAware/'+ x +'sequentialCarbonAwareBaseline.csv', 'a')
    ...
    print(n_HP, "; ", n_NP, "; ", n_LP, "; ", totRevenue)
