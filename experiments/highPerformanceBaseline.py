...
for inputVersion in range(4):
    ...
    f = open('results/' + y + '/highPerformanceBaseline_' + y + '.csv', 'w')
    writer = csv.writer(f)
    row = ["Energy demand", "QoE", "Revenue per user", "Revenue", "Revenue in percentage"]
    writer.writerow(row)
    f.close()
    f = open('results/' + y + '/highPerformanceBaseline_' + y + '.csv', 'a')
    writer = csv.writer(f)
